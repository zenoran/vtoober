...
class BasicMemoryAgent(AgentInterface):
        ...
        if use_mcpp:
            if self._mcp_server_manager and self._tool_manager:
                ...
                if not self._json_detector:
                     logger.warning("MCP Plus enabled but JSON Detector failed to initialize (needed for prompt mode)." )
            else:
                logger.warning("MCP Plus is enabled but ToolManager or ServerManager failed to initialize.")
        else:
    ...
    def _to_messages(self, input_data: BatchInput) -> List[Dict[str, Any]]:
        ...
        if user_content:
            ...
            self._add_message(text_prompt if text_prompt else "[User provided image(s)]", "user")
        else:
            ...
            for i, call in enumerate(tool_calls):
                 ...
                 if caller_mode == "Claude":
                     results.append({
                      ...
                      results.append({
                         "role": "tool", "tool_call_id": tool_id, "content": error_content
                     })
            return results

        logger.info(f"Attempting to execute {len(tool_calls)} tool(s) for {caller_mode} caller.")
        async with MCPClient(self._mcp_server_manager) as client:
            for call in tool_calls:
                tool_name, tool_id, tool_input, is_error, result_content, parse_error = self._parse_tool_call(call)

                if parse_error:
                    logger.warning(f"Skipping tool call due to parsing error: {result_content}")
                else:
                    # Execute the tool
                    is_error, result_content = await self._run_single_tool(client, tool_name, tool_id, tool_input)

                # Format the result based on the caller mode
                formatted_result = self._format_tool_result(caller_mode, tool_id, result_content, is_error)
                if formatted_result:
    ...
    def _parse_tool_call(self, call: Union[Dict[str, Any], ToolCallObject]) -> tuple:
        ...
        if isinstance(call, ToolCallObject):
            ...
            try:
                ...
                logger.error(f"Failed to decode OpenAI tool arguments for '{tool_name}': {call.function.arguments}")
    ...
    async def _run_single_tool(self, client: MCPClient, tool_name: str, tool_id: str, tool_input: Any) -> tuple[bool, str]:
        """Runs a single tool using the MCPClient."""
        logger.info(f"Executing tool: {tool_name} (ID: {tool_id}) with input: {tool_input}")
        ...
        if not tool_info:
            ...
            try:
                ...
                logger.exception(f"Error executing tool '{tool_name}' (ID: {tool_id}): {e}")
                result_content = f"Error executing tool '{tool_name}': {e}"
                is_error = True
        return is_error, result_content

    def _format_tool_result(self, caller_mode: Literal["Claude", "OpenAI"], tool_id: str, result_content: str, is_error: bool) -> Dict[str, Any] | None:
         ...
         if caller_mode == "Claude":
             ...
             logger.debug(f"Formatting result for Prompt mode. ID: {tool_id}, Error: {is_error}")
             return {
    ...
    async def _claude_tool_interaction_loop(
        ...
        while True:
            ...
            if pending_tool_calls:
                ...
                tool_results = await self._execute_tools(pending_tool_calls, caller_mode="Claude") # Pass mode

                if not tool_results:
                    logger.error("Claude tool execution failed to produce results. Stopping interaction.")
                    return

                # Append user message with tool results (already formatted by _execute_tools)
                messages.append({"role": "user", "content": tool_results})
                logger.debug(f"Appended Claude tool results. New message count: {len(messages)}")
    ...
    def _process_tool_from_prompt_json(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            if all([server, tool_name, arguments_str]):
                 try:
                     ...
                     logger.info(f"Parsed tool call from prompt JSON: {tool_name}")
                      ...
                      logger.error(f"Failed to decode arguments JSON in prompt mode tool call: {arguments_str}")
                 except Exception as e:
                      logger.error(f"Error processing prompt mode tool dict: {item} - {e}")
            else:
                 logger.warning(f"Skipping invalid tool structure in prompt mode JSON: {item}")
        return parsed_tools
    ...
    async def _openai_tool_interaction_loop(
        ...
        while True:
            ...
            logger.debug(f"Calling OpenAI compatible API. Prompt Mode: {self.prompt_mode_flag}. Message count: {len(messages)}")
            stream = self._llm.chat_completion(messages, current_system_prompt, tools=tools_for_api)
            pending_tool_calls.clear()
                ...
                if self.prompt_mode_flag:
                     ...
                     if isinstance(event, str):
                         ...
                         if self._json_detector:
                              ...
                              if potential_json:
                                   logger.info("Detected potential JSON tool call in prompt mode stream.")
                                   ...
                                   try:
                                        ...
                                        if isinstance(potential_json, list):
                                             ...
                                             logger.warning(f"JSON detector returned unexpected type: {type(potential_json)}")

                                        if detected_prompt_json:
                                             logger.debug(f"Successfully parsed JSON: {detected_prompt_json}")
                                             # Stop processing stream for this turn, proceed to tool execution
                                             break
                                   except Exception as e:
                                        logger.error(f"Error fully parsing detected JSON: {e}")
                                        self._json_detector.reset() # Reset detector on error
                         yield event # Yield text chunk regardless
                     else:
                          logger.warning(f"Received non-string event in prompt mode stream: {type(event)}")
                else:
                     ...
                     if isinstance(event, str):
                         ...
                         logger.info(f"OpenAI LLM requested {len(pending_tool_calls)} tool(s) via native API.")
                         ...
                         logger.warning(f"LLM {getattr(self._llm, 'model', '')} reported no native tool support. Switching to prompt mode.")
                         ...
                         goto_next_while_iteration = True # Use a flag to break and continue
                         break
                     else:
                         logger.warning(f"Received unexpected event type from native OpenAI stream: {type(event)}")
            ...
            if detected_prompt_json:
                ...
                if parsed_tools:
                     tool_results_data = await self._execute_tools(parsed_tools, caller_mode='Prompt')
                     if tool_results_data:
                          # Format results into a single user message string
                          result_strings = [res.get('content', 'Error: Malformed result') for res in tool_results_data]
                          combined_results_str = "\n".join(result_strings)
                          messages.append({"role": "user", "content": combined_results_str})
                          logger.debug(f"Appended prompt mode tool results as user message. New message count: {len(messages)}")
                     else:
                          logger.error("Prompt mode tool execution failed to produce results. Stopping interaction.")
                          return
                else:
                    logger.warning("Detected JSON but failed to parse into valid tool calls.")
                ...
                continue
                 ...
                 tool_results_messages = await self._execute_tools(pending_tool_calls, caller_mode='OpenAI')

                 if not tool_results_messages:
                     logger.error("OpenAI native tool execution failed to produce results. Stopping interaction.")
                     return

                 messages.extend(tool_results_messages)
                 logger.debug(f"Appended OpenAI native tool results. New message count: {len(messages)}")
                 continue # Continue loop after native tool execution

            else:
                # --- No Tool Calls, Final Response --- 
                logger.info("No tool calls requested or fallback executed. Interaction complete for this turn.")
                if current_turn_text:
                    self._add_message(current_turn_text, "assistant")
                else:
                    logger.info("No text generated in the final response.")
        ...
        async def chat_with_memory(input_data: BatchInput) -> AsyncIterator[str]:
            ...
            if self._use_mcpp and self._tool_manager and self._mcp_server_manager:
                if isinstance(self._llm, ClaudeAsyncLLM):
                    ...
                    logger.info("LLM is OpenAI compatible type, preparing OpenAI tools (will check support via API).")
                    tool_mode = "OpenAI"
                    tools = self._tool_manager.get_all_tools(mode=tool_mode)
                    # We don't know native support yet, _openai_tool_interaction_loop will find out
                    llm_supports_native_tools = True # Assume true initially, loop corrects if needed
                else:
                    logger.warning(f"Tool use enabled, but LLM type {type(self._llm)} not explicitly handled. Proceeding without tools.")

                if llm_supports_native_tools and not tools:
                     logger.warning(f"Tool Manager returned no tools for mode '{tool_mode}'. Tool usage might be limited.")
                     # Don't disable llm_supports_native_tools here, allow fallback check

            # --- Execute Correct Interaction Loop or Fallback ---
            if self._use_mcpp and tool_mode == "Claude": # Claude only uses native tools
                logger.info("Starting chat with Claude tool support.")
                async for text_chunk in self._claude_tool_interaction_loop(messages, tools if tools else []):
                    yield text_chunk
                return
            elif self._use_mcpp and tool_mode == "OpenAI": # OpenAI uses native or fallback
                 logger.info("Starting chat with OpenAI compatible LLM (native or prompt tools).")
                 async for text_chunk in self._openai_tool_interaction_loop(messages, tools if tools else []):
                      yield text_chunk
                 return
            else:
                 # --- Fallback/Non-Tool Path (or unknown LLM type with MCP+) --- 
                logger.info("Starting simple chat completion (MCP disabled, unknown LLM, or no tools).")
                ...
                async for event in token_stream:
                    ...
                    if isinstance(event, dict) and event.get("type") == "text_delta":
                        ...
                        logger.warning(f"Received unexpected event type in simple stream: {type(event)}")
                        continue
                    if text_chunk:
                        yield text_chunk
