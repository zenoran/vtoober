...
class BasicMemoryAgent(AgentInterface):
    ...
    async def _execute_tools(
        ...
        if not self._mcp_client or not self._tool_manager:
            logger.error("MCP Client or Tool Manager not available for this agent.")
        ...
        logger.info(f"Finished executing tools with {len(tool_results_for_llm)} results.")
    ...
    def _parse_tool_call(self, call: Union[Dict[str, Any], ToolCallObject]) -> tuple:
        ...
        if isinstance(call, ToolCallObject):
            ...
            try:
                ...
                logger.error(f"Failed to decode OpenAI tool arguments for '{tool_name}'")
        ...
        for item in data:
            ...
            if all([server, tool_name, arguments_str]):
                try:
                    ...
                    logger.error(f"Failed to decode arguments JSON in prompt mode tool call")
                except Exception as e:
