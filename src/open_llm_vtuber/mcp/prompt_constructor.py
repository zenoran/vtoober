...
class PromptConstructor:
    ...
    def __init__(self, prompts_path: str | Path = DEFAULT_PROMPTS_PATH) -> None:
        ...
        try:
            ...
            logger.error(f"PC: File '{prompts_path}' does not exist, or is not a json file.")
            raise ValueError(f"PC: File '{self.prompts_path}' does not exist, or is not a json file.")
        
        self.prompts: Dict[str, str] = json.loads(self.prompts_path.read_text(encoding="utf-8"))
        ...
        for server_name in self.server_manager.servers.keys():
            ...
            async with MCPClient(self.server_manager) as client:
                try:
                    ...
                    logger.error(f"PC: Failed to connect to server '{server_name}': {e}")
    ...
    def construct_prompt(self, force: bool = False) -> None:
        ...
        for server_name, tools in self.servers_info.items():
            if server_name in self.prompts and not force:
                logger.info(f"PC: Prompt for server '{server_name}' already exists. Skipping...")
                continue
            ...
            for tool_name, tool_info in tools.items():
                ...
                for param_name, param_info in tool_info["parameters"].items():
                    ...
                    description = description if description else param_info.get("title", "No description provided.")
                    prompt += f"                {param_name}:\n"
                    prompt += f"                    Type: {param_info['type']}\n"
                    prompt += f"                    Description: {description}\n"
                if tool_info["required"]:
                    prompt += f"            Required: {', '.join(tool_info['required'])}\n"
    ...
    def dump_prompts(self) -> None:
        ...
        self.prompts_path.write_text(json.dumps(self.prompts, indent=4), encoding="utf-8")
