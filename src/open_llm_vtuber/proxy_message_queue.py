...
class ProxyMessageQueue:
    ...
    def queue_message(self, message: Dict, sender_id: Optional[str] = None) -> None:
        ...
        logger.info(f"Queuing message: {message.get('text', '')} (active conversation: {self._conversation_active})")
            ...
            while self._running:
                ...
                async with self.lock:
                    if not self._conversation_active and self.has_pending_messages():
                        ...
                        logger.info(f"Consumer processing message: {message.get('text', '')}")
                ...
                if not self.has_pending_messages() and not self._conversation_active:
                    ...
                    if not self.has_pending_messages() and not self._conversation_active:
                        logger.debug("No more pending messages and no active conversation, pausing consumer")
                        break
    ...
    async def _forward_message(self, message: Dict, sender_id: Optional[str] = None):
        ...
        try:
            if self._forward_func:
                # If this is a text input, broadcast transcription before processing
                if message.get("type") == "text-input":
                    ...
                    # Forward transcription to all clients except sender
